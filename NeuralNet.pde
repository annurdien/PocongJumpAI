class NeuralNet {
  ...
  void show(float x, float y, float w, float h, float[] vision, float[] decision) {
     ...
     text("Kiri",x+(lc*nSize)+(lc*nSpace)+nSize+5,y+oBuff+(nSize/2));
     text("Diam",x+(lc*nSize)+(lc*nSpace)+nSize+5,y+oBuff+space+nSize+(nSize/2));
     text("kanan",x+(lc*nSize)+(lc*nSpace)+nSize+5,y+oBuff+(2*space)+(2*nSize)+(nSize/2));
  }
}
